// Styles for the mapbox-streets-v6 vector tile data set. Loosely based on http://a.tiles.mapbox.com/v4/mapbox.mapbox-streets-v6.json

// Test 
// Work in progress !!

export function createVectorStyle(Style, Fill, Stroke, Icon, Text, jsonConfig) {
  var fill = new Fill({
    color: ""
  });
  ...
  return function(feature, resolution) {
    ...
    var getStyleConfig = layer => {
      return jsonConfig.layers.filter(el => {
        return el.id == layer;
      })[0];
    };
    var styleConfig = getStyleConfig(layer);

    switch (styleConfig.type) {
      case "fill":
        fill.setColor(styleConfig.paint);
        break;
      case "symbol":
